...
@section('content')
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="p-8">
            <h1 class="text-2xl font-semibold font-Poppins text-gray-800">Tambah Tiket</h1>
            <div class="mt-4 p-8" id="ticketForm" >
                <form id="ticketFormElement" method="POST" action="/admin/tickets" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="deskripsi" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm" name="deskripsi" required></textarea>
                        </div>
                        <div>
                            ...
                            <select id="kategori_tiket_id" name="kategori_tiket_id"
                                ...
                                @foreach ($kategoriTiket as $kategori)
                                    <option value="{{ $kategori->kategori_tiket_id }}">{{ $kategori->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="md:col-span-2">
                            <x-input-label for="gambar" :value="__('Gambar')" />
                            <input id="gambar" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm"
                                type="file" name="gambar" />
                        </div>
                        <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6" id="tanggalFields"
                            style="display: none;">
                            <div>
                        ...
                        <div>
                            ...
                            <x-text-input id="kuota" class="block mt-1 w-full" type="number" name="kuota" required />
                        </div>
                        <div>
                    ...
                    <div class="mt-6 flex justify-end">
                        ...
                        <button type="submit"
                            class="bg-teal-500 hover:bg-teal-600 text-white font-semibold py-2 px-4 rounded-md shadow-md">Simpan</button>
                    </div>
                </form>
            </div>
    </div>
    ...
    <script>
           document.getElementById('kategori_tiket_id').addEventListener('change', function() {
            var selectedCategory = this.value;
            var tanggalFields = document.getElementById('tanggalFields');
            var selectedOption = this.options[this.selectedIndex];
            if (selectedOption.text.toLowerCase().includes('event')) {
