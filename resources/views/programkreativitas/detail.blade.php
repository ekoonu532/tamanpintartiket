<x-app-layout>
    ...
    <div class="min-h-screen py-16  items-center justify-center">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row md:space-x-4 space-y-4 md:space-y-0">
            ...
            <div class="w-full md:w-3/5 border-r shadow-md bg-white pr-4 py-4 rounded-lg mt-6">
                ...
                <div class="px-4">
                    <div class="h-60 md:h-80 rounded-lg bg-gray-100 mb-4">
                        <figure class="h-full w-full">
                            <img src="{{ asset('storage/' . $tiket->gambar) }}" alt="{{ $tiket->nama }}" class="h-full w-full object-cover transition duration-500 group-hover:scale-105">
                        </figure>
                    </div>
                </div>
                ...
                <div class="flex flex-col space-y-4 px-4">
                    <h2 class="mb-2 ml-4 leading-tight tracking-tight font-Poppins font-bold text-gray-800 text-2xl md:text-3xl">
                        {{ $tiket->nama }}
                    </h2>
                    <p class="text-gray-500 ml-4 text-sm">By <a href="#" class="text-indigo-600 hover:underline">Nama Taman Pintar, Yogyakarta</a></p>

                    <div class="flex flex-col space-y-2">
                        <div class="mb-2 ml-4">
                            <p class="text-gray-900 text-xl font-bold font-Poppins">Harga: <span class="text-teal-400 text-xl font-bold font-Poppins">Rp {{ number_format($tiket->harga_anak) }}</span> </p>
                            {{-- <p class="text-gray-900 text-xl font-bold font-Poppins">Harga Dewasa: <span class="text-teal-400 text-xl font-bold font-Poppins"> Rp {{ number_format($tiket->harga_dewasa) }}</span></p> --}}
                        </div>
            ...
            <div class="w-full  md:w-2/5">
                ...
                <div class="border rounded-lg mt-6 bg-white p-4">
                    ...
                    <form action="{{ route('order') }}" method="POST">
                        @csrf
                        <select id="hari_tanggal" name="hari_tanggal" class="w-full border-gray-300 rounded-md shadow-md p-2 bg-white focus:border-teal-400 focus:ring focus:ring-teal-400 focus:ring-opacity-50">
                            <option value="" class="bg-white font-Poppins text-gray-900" disabled selected>Pilih salah satu</option>
                            @for ($i = 0; $i < 7; $i++)
                              @php
                                $hari_indonesia = [
                                  'Senin',
                              ...
                              @endphp
                              <option value="{{ $tanggal->toDateString() }}">{{ $hari }} {{ $tanggal_lengkap }}</option>
                            @endfor
                          </select>
                    </form>
                            ...
                            .then(data => {
                                if (data.kuota > 0) {
                                    ...
                                    footnote.textContent =
                                        'Klik tombol "Pesan Sekarang" untuk melanjutkan proses pemesanan tiket.';
